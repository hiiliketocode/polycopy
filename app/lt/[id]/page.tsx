    ...
    <div className="container mx-auto py-8 px-4 max-w-7xl">
      ...
      <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)}>
        ...
        <TabsContent value="settings">
          <div className="space-y-6">
            {/* Activation Controls */}
            <Card className={strategy.is_active ? 'border-green-200 bg-green-50/30' : 'border-amber-200 bg-amber-50/30'}>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  Strategy Activation
                  {settingsMsg && (
                    <span className="text-sm font-normal text-green-600">{settingsMsg}</span>
                  )}
                </CardTitle>
                <CardDescription>
                  {strategy.is_active 
                    ? 'Strategy is active. The cron will execute trades when FT signals appear.' 
                    : 'Strategy is inactive. Activate it to start executing trades.'}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex flex-wrap items-center gap-4">
                  <div className="flex items-center gap-3">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={(e) => patchStrategy({ is_active: e.target.checked })}
                  ...
                  <div className="flex items-center gap-3">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={strategy.shadow_mode}
                        onChange={(e) => patchStrategy({ shadow_mode: e.target.checked })}
                        disabled={savingSettings}
                        className="sr-only peer"
                      />
                    ...
                    <span className="text-sm font-medium">
                      Shadow Mode {strategy.shadow_mode ? '(ON — no real orders)' : '(OFF — real orders)'}
                    </span>
                  </div>
                </div>
                ...
                {!strategy.is_active && (
                  <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-sm text-amber-800">
                    <strong>Strategy is inactive.</strong> Toggle the switch above to start executing trades. 
                    Make sure your risk settings and capital are configured first.
                    {strategy.shadow_mode && ' Shadow mode is on — trades will be simulated, not placed on Polymarket.'}
                  </div>
                )}
                {strategy.is_active && !strategy.shadow_mode && (
                  <div className="p-3 rounded-lg bg-green-50 border border-green-200 text-sm text-green-800">
                    <strong>Live execution is ON.</strong> Real orders will be placed on Polymarket when FT signals are detected.
                  </div>
                )}
                {strategy.is_active && strategy.shadow_mode && (
                  <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 text-sm text-blue-800">
                    <strong>Shadow mode is active.</strong> Trades will be simulated and logged but no real CLOB orders will be placed.
                  </div>
                )}
              </CardContent>
            </Card>

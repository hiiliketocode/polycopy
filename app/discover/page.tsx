...
function Top30PerformanceChart({ series }: { series: { date: string; value: number }[] }) {
  const chartData = useMemo(() => series.filter((entry) => entry && entry.date && Number.isFinite(entry.value)), [series]);
  if (chartData.length === 0) {
    return <div className="text-sm text-slate-500">No performance data yet.</div>;
  }

  const chartWidth = Math.max(360, Math.min(760, chartData.length * 26));
  ...
  return (
    <div className="w-full">
      <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-[180px]">
        <defs>
          <linearGradient id="top30-performance-area" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#0f766e" stopOpacity="0.15" />
            <stop offset="100%" stopColor="#0f766e" stopOpacity="0" />
          </linearGradient>
          <linearGradient id="top30-performance-stroke" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#0f766e" />
            <stop offset="100%" stopColor="#158b6f" />
          </linearGradient>
        </defs>
        <path d={areaPath} fill="url(#top30-performance-area)" />
        <path
          d={path}
          stroke="url(#top30-performance-stroke)"
          strokeWidth="2.2"
          fill="none"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <circle cx={latestPoint.x} cy={latestPoint.y} r="4" fill="#0f766e" stroke="#fff" strokeWidth="2" />
      </svg>
    </div>
  );
}
...
function Top30WindowChart({
  ...
  if (chartData.length === 0) {
    return <div className="text-sm text-slate-500">No realized P&L data available for this window.</div>;
  }
  ...
  const barGap = chartData.length <= 2 ? '10%' : '25%';
  ...
  return (
    <div className="h-72 w-full animate-in fade-in duration-700 sm:h-80">
      <ResponsiveContainer width="100%" height="100%">
        {mode === 'daily' ? (
          <BarChart data={chartData} barSize={barSize} barCategoryGap={barGap}>
...
function DiscoverPageContent() {
  ...
  const [top30Series, setTop30Series] = useState<Array<{ date: string; value: number }>>([]);
  const [top30DailyRows, setTop30DailyRows] = useState<Array<Record<string, any>>>([]);
  const [top30Windows, setTop30Windows] = useState<Record<string, RealizedIndexWindow>>({});
  const [loadingTop30Series, setLoadingTop30Series] = useState(true);
  const [top30SeriesError, setTop30SeriesError] = useState<string | null>(null);
  const [selectedWindowKey, setSelectedWindowKey] = useState<typeof TOP30_TIME_WINDOW_OPTIONS[number]['key']>('30D');
  ...
  useEffect(() => {
    const fetchTraders = async () => {
      ...
      try {
        ...
        const tradersWithROI = (data.traders || []).map((trader) => ({
          ...trader,
  ...
  useEffect(() => {
    ...
    const loadTop30Series = async () => {
      setLoadingTop30Series(true);
      setTop30SeriesError(null);
      try {
        const response = await fetch('/api/realized-pnl/index', { cache: 'no-store' });
      if (!response.ok) {
        throw new Error('Failed to load Top 30 performance');
      }
      const payload = await response.json();
      if (cancelled) return;
      const rawSeries = Array.isArray(payload?.series) ? payload.series : [];
      setTop30DailyRows(rawSeries);
      ...
      const windowsMap = windows.reduce<Record<string, RealizedIndexWindow>>((acc, window) => {
        if (window?.window_key) {
          acc[window.window_key] = window;
        }
        return acc;
      }, {});
      setTop30Windows(windowsMap);
      } catch (error) {
        if (!cancelled) {
          setTop30Series([]);
          setTop30SeriesError(error instanceof Error ? error.message : 'Failed to load Top 30 performance data');
        }
      } finally {
        if (!cancelled) {
          setLoadingTop30Series(false);
        }
      }
    };
  ...
  const normalizedTop30DailyRows = useMemo(() => {
    return top30DailyRows
      .map((row) => ({
  ...
  const top30WindowRows = useMemo(() => {
    if (normalizedTop30DailyRows.length === 0) return [];
    if (selectedWindowKey === '1D') {
      const latest = normalizedTop30DailyRows[normalizedTop30DailyRows.length - 1];
      return latest ? [latest] : [];
    }
    const windowData = top30Windows[selectedWindowKey];
    if (!windowData) return normalizedTop30DailyRows;
    const startDate = windowData.start_date;
    const endDate = windowData.as_of;
    return normalizedTop30DailyRows.filter((row) => {
      if (startDate && row.date < startDate) return false;
      if (endDate && row.date > endDate) return false;
      return true;
    });
  }, [selectedWindowKey, normalizedTop30DailyRows, top30Windows]);

  const averageChartRows = useMemo(
    () =>
      top30WindowRows.map((row) => ({
        date: row.date,
  ...
  const latestTop30Point = top30Series[top30Series.length - 1];
  const latestTop30Label = latestTop30Point
    ? (() => {
        const parsedDate = Date.parse(latestTop30Point.date);
    ...
    : null;
  const latestNormalizedDailyRow = normalizedTop30DailyRows[normalizedTop30DailyRows.length - 1] ?? null;
  const selectedWindowData = selectedWindowKey === '1D' ? undefined : top30Windows[selectedWindowKey];
  const windowRangeLabel =
    selectedWindowKey === '1D'
      ? latestNormalizedDailyRow
        ? formatShortDateLabel(latestNormalizedDailyRow.date)
        : 'Yesterday'
      : formatDateRangeLabel(selectedWindowData?.start_date ?? null, selectedWindowData?.as_of ?? null);
  const latestTop30ValueLabel = latestTop30Point ? formatSignedLargeNumber(latestTop30Point.value) : '—';
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white md:pt-0 pb-20 md:pb-8">
        ...
        <div className="bg-white">
          <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-8 sm:py-10 space-y-6">
            <div className="mb-4 flex flex-wrap items-center gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Top 30 Windows</p>
                <h3 className="text-lg font-semibold text-slate-900">Top 30 Realized P&L</h3>
              </div>
              <div className="ml-auto flex flex-wrap gap-2">
                {TOP30_TIME_WINDOW_OPTIONS.map((option) => (
                  <button
                    key={option.key}
              ...
              <div className="space-y-6">
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                  <div className="px-6 py-5 border-b border-slate-100">
                    <div className="flex flex-wrap items-center gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">Average Realized P&L</p>
                        <p className="text-xs text-slate-500">{windowRangeLabel}</p>
                      </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                  <div className="px-6 py-5 border-b border-slate-100">
                    <div className="flex flex-wrap items-center gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">Aggregate Realized P&L</p>
                        <p className="text-xs text-slate-500">{windowRangeLabel}</p>
                      </div>
                      <div className="ml-auto flex flex-wrap items-center gap-2">
                        <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 p-1 shadow-sm">
        ...
        <div className="bg-slate-50">
          <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-8 sm:py-12">
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="px-6 py-5 border-b border-slate-100">
                <h3 className="text-lg font-semibold text-slate-900">Top 30 Combined Performance</h3>
                <p className="text-xs text-slate-500">Cumulative realized P&L for the active top 30 wallets</p>
              </div>
              <div className="px-6 py-6 space-y-4">
                {loadingTop30Series ? (
                  <div className="text-sm text-slate-500">Loading chart...</div>
                ) : top30SeriesError ? (
                  <div className="text-sm text-slate-500">{top30SeriesError}</div>
                ) : (
                  <>
                    <div className="flex flex-wrap items-baseline gap-3">
                      <span className="text-sm text-slate-500">Latest cumulative P&L</span>
                      <span className="text-3xl font-semibold text-slate-900">{latestTop30ValueLabel}</span>
                      {latestTop30Label && (
                        <span className="text-xs text-slate-400 uppercase tracking-[0.2em]">{latestTop30Label}</span>
                      )}
                    </div>
                    <Top30PerformanceChart series={top30Series} />
                  </>
                )}
              </div>
            </div>

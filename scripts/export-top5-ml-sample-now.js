...
/**
 * Export a ML-ready dataset for top5_traders_trades as CSV (joined + timing),
 * without requiring any DB views/migrations to be applied.
 *
 * It:
 * - Reads trades from public.top5_traders_trades in batches (PostgREST limit is ~1000)
 ...
 *      IMPORTANT: close_time is when market stops accepting trades
...
const OUTPUT = process.argv.find((a) => a.startsWith('--output='))?.split('=')[1] || `top5_ml_${Date.now()}.csv`
...
async function getTradeColumns() {
  const { data, error } = await supabase.from('top5_traders_trades').select('*').limit(1)
  if (error) throw error
  if (!data || data.length === 0) throw new Error('No rows in top5_traders_trades')
...
async function main() {
  ...
  console.log('📤 Exporting top5 ML CSV (joined + timing)')
  console.log('='.repeat(60))
  ...
  while (written < target) {
    ...
    for (const t of trades) {
      ...
      const closeTimeMs = toMs(m?.close_time) // IMPORTANT: close_time when trading stops

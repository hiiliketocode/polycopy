...
- **Trader pool (shared):** Leaderboard (month PNL, month VOL, week PNL, day VOL) with **11 pages per view** (offset 0–1000, 100 per page → up to 1,100 per view, 4 views → up to ~4,400 slots) merged by wallet, plus any `target_trader` / `target_traders` from wallet configs. So the unique set is typically **several thousand** traders, not just “top 100”.
...
| Trader pool | Same file: 4 leaderboard views × 11 pages (100 each, offset 0–1000) + `target_trader` / `target_traders` per wallet |

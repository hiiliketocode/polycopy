-- ============================================================================
-- INCREMENTAL UPDATE: i_wish_i_copied_that
-- Purpose: Add only NEW trades that haven't been processed yet
-- Strategy: INSERT trades where trade_id doesn't exist in table
-- Run: Daily (after markets close)
-- ============================================================================

-- First, add tracking column if it doesn't exist
...
-- Insert only new trades (those not already in table)
INSERT INTO `gen-lang-client-0299056258.polycopy_v1.i_wish_i_copied_that`
(
  trade_id,
  condition_id,
  wallet_address,
...
-- Step 4: Filter for Human trades
filtered_trades AS (
  SELECT 
    wt.*,
  ...
  WHERE wt.roi_pct >= 500.0
    AND wt.invested_usd >= 50.0
    AND tmv.total_invested_usd >= 100.0
    AND tmv.total_invested_usd <= 10000000.0
),

-- Step 5: Get trader stats
...
-- Step 7: Calculate total excitement score and rank
ranked_trades AS (
  SELECT 
    *,
    ...
    ) as trade_rank_per_market
  FROM scored_trades
),

-- Step 8: Final selection (same as main query)
new_trades AS (
  SELECT 
    trade_id,
    condition_id,
...
-- Only insert trades that don't already exist
SELECT 
  nt.*,
  CURRENT_TIMESTAMP() as created_at,
  NULL as last_processed_at  -- New trades haven't been processed yet
FROM new_trades nt
WHERE NOT EXISTS (
  SELECT 1 
  FROM `gen-lang-client-0299056258.polycopy_v1.i_wish_i_copied_that` existing
  WHERE existing.trade_id = nt.trade_id
)
ORDER BY excitement_score DESC;

-- Cleanup: Remove trades older than 7 days
DELETE FROM `gen-lang-client-0299056258.polycopy_v1.i_wish_i_copied_that`
WHERE market_close_time < TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 7 DAY);

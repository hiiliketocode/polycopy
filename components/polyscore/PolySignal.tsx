...
function calculateSignal(
  ...
  const insights: InsightData = {
    ...
    profileTrades: stats?.profile_trades_count ?? stats?.trade_count ?? drawer?.competency?.total_trades ?? 0,
    globalTrades: stats?.global_trades_total ?? 0,
    
    profileWinRate: stats?.profile_win_rate ?? stats?.trader_win_rate ?? drawer?.competency?.niche_win_rate ?? null,
    globalWinRate: stats?.global_win_rate ?? drawer?.competency?.global_win_rate ?? null,
    
    profileRoiPct: stats?.profile_roi_pct ?? stats?.trader_historical_roi_pct ?? null,
    globalRoiPct: stats?.global_roi_pct ?? null,
    profileAvgPnl: stats?.profile_avg_usd ?? null,
    globalAvgPnl: stats?.global_avg_usd ?? null,
    
    convictionMultiplier: stats?.conviction_multiplier ?? null,
    positionConviction: stats?.position_conviction ?? null,
    tradeConviction: stats?.trade_conviction ?? null,
    exposureUsd: stats?.exposure ?? drawer?.conviction?.total_exposure_usd ?? null,
    zScore: drawer?.conviction?.z_score ?? data.analysis?.debug?.z_score ?? 0,
    isOutlier: drawer?.conviction?.is_outlier ?? false,
...
export function PolySignal({ data, loading, entryPrice, currentPrice }: PolySignalProps) {

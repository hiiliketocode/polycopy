...
export function UpgradeModal({ open, onOpenChange }: UpgradeModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[min(520px,calc(100vw-2rem))] gap-0 border-0 bg-poly-cream p-0 overflow-hidden max-h-[90vh] overflow-y-auto">
        <DialogTitle className="sr-only">Upgrade to Premium</DialogTitle>

        {/* Header — yellow block */}
        <div className="bg-poly-yellow px-8 pb-6 pt-8">
          <p className="mb-1 font-sans text-xs font-bold uppercase tracking-widest text-poly-black/50">
            PREMIUM_ACCESS
          </p>
        ...
        <div className="border-b border-border bg-white px-8 py-6">
          ...
          <p className="mt-1 font-sans text-xs font-bold uppercase tracking-widest text-muted-foreground">
        ...
        <div className="border-t border-border bg-white px-8 py-6 pb-24 sm:pb-6">
          <button
            ...
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 bg-poly-black py-4 font-sans text-xs font-bold uppercase tracking-[0.2em] text-poly-yellow transition-all hover:bg-poly-yellow hover:text-poly-black disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              "PROCESSING..."
            ) : (
          ...
          <p className="mt-3 text-center font-sans text-[11px] font-bold uppercase tracking-widest text-muted-foreground">

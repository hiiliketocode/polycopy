-- ============================================================================
-- Rebuild Trader Global Stats Table (FIXED)
...
unique_markets AS (
  SELECT condition_id, status, winning_label
...
stats_by_wallet AS (
  SELECT 
    ...
    -- FIX: Only sum invested amount for RESOLVED trades (for accurate ROI)
    SUM(CASE WHEN is_resolved THEN trade_size_usd ELSE 0 END) as L_resolved_invested_usd,
    -- Keep total invested for reference (all trades)
    ...
    -- Last 30 days: ALL trades in window
    ...
    -- Last 7 days: ALL trades in window
...
SELECT 
  ...
  D30_count,
  D7_count,
  -- Resolved counts (new - for transparency)
  L_resolved_count,
  D30_resolved_count,
  D7_resolved_count,
  -- Lifetime win rates (based on resolved trades)
  CASE 
  ...
  -- FIX: ROI now uses RESOLVED trades' invested amount as denominator
  ...
  -- Average PnL per trade (resolved trades only - matches numerator)
  ...
  -- Total invested (all trades - kept for reference/context)
  COALESCE(L_total_invested_usd, 0.0) as L_total_invested_usd,
  COALESCE(D30_total_invested_usd, 0.0) as D30_total_invested_usd,
  COALESCE(D7_total_invested_usd, 0.0) as D7_total_invested_usd,
  -- Resolved invested (for ROI calculation verification)

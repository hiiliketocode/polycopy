-- Fix: Invested USD should reflect the actual spent amount when trades fill
-- This ensures partial fills show the dollars that actually left the wallet (filled_size × price),
-- while still falling back to stored amount_invested for manual copies or missing fills.
-- Also keeps the existing exit price heuristics and security_invoker flag from the previous migration.
  ...
  SELECT
    ...
    CASE
      WHEN o.filled_size IS NOT NULL 
        AND o.filled_size > 0 
        AND COALESCE(o.price_when_copied, o.price) IS NOT NULL
      THEN COALESCE(o.price_when_copied, o.price) * o.filled_size
      WHEN o.filled_size IS NULL 
...
COMMENT ON VIEW public.orders_copy_enriched IS
  'Normalized copy-trade view with canonical entry/exit/invested/side and derived PnL fields. Invested USD prioritizes actual filled quantity.';

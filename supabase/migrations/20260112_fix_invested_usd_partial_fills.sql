-- Fix invested_usd to use ACTUAL filled amount, not intended amount
-- Critical fix: For partial fills, we must show what was actually spent
...
WITH base AS (
  SELECT
    ...
    -- This ensures partial fills show ACTUAL amount spent, not intended
    CASE
...
WHERE NOT (
  ...
  AND lower(coalesce(base.time_in_force, base.order_type, '')) != 'gtc'
);

COMMENT ON VIEW public.orders_copy_enriched IS
  'Normalized copy-trade view with ACCURATE invested_usd based on actual filled amount (not intended amount).';
